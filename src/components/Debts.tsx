...
import { DollarSign, Search, X, Pencil, Trash2, AlertTriangle, Download } from 'lucide-react';
import { formatCurrency, formatDate, PAYMENT_METHODS, exportToCSV } from '@/lib/helpers';
...
export default function Debts() {
  ...
  async function loadData() {
    try {
      ...
      setDebtors(data.debtors); setPayments(data.payments);
    } catch { showError('خطا در بارگذاری'); }
    setLoading(false);
  }
  ...
  async function handlePayment() {
    ...
    try {
      await fetch('/api/payments', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ customer_id: showPay.id, amount: payAmount, method: payMethod, note: payNote }) });
      success('پرداخت ثبت شد');
      setShowPay(null); setPayAmount(0); setPayNote('');
      await loadData();
    } catch { showError('خطا در ثبت پرداخت'); }
  }
  ...
  async function saveEditPay() {
    if (!editPay) return;
    try {
      await fetch('/api/payments', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: editPay.id, ...editForm }) });
      success('پرداخت ویرایش شد');
      setEditPay(null); await loadData();
    } catch { showError('خطا در ویرایش'); }
  }

  async function handleDeletePay() {
    if (!confirmDelete) return;
    try {
      await fetch('/api/payments', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: confirmDelete.id }) });
      success('پرداخت حذف شد');
      setConfirmDelete(null); await loadData();
    } catch { showError('خطا در حذف'); }
  }
  ...
  return (
    <div className="p-4 space-y-4 h-full overflow-y-auto">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <h2 className="text-lg font-bold">💳 مدیریت بدهی‌ها</h2>
        <button className="btn btn-outline btn-sm gap-1" onClick={() => exportToCSV(debtors, 'debtors', [{key:'name',label:'نام'},{key:'phone',label:'تلفن'},{key:'balance',label:'بدهی'}])}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-3 animate-fade-in">
        <div className="card bg-error/10 border border-error/30"><div className="card-body p-3"><p className="text-xs text-base-content/60">مجموع بدهی</p><p className="font-bold text-error text-lg">{formatCurrency(totalDebt)}</p></div></div>
        <div className="card bg-base-200 border border-base-300"><div className="card-body p-3"><p className="text-xs text-base-content/60">تعداد بدهکاران</p><p className="font-bold text-lg">{filtered.length}</p></div></div>
        <div className="card bg-success/10 border border-success/30"><div className="card-body p-3"><p className="text-xs text-base-content/60">پرداخت‌ها</p><p className="font-bold text-success text-lg">{payments.length}</p></div></div>
      </div>

      <div className="card bg-base-200 animate-fade-in"><div className="card-body p-4">
        ...
        <div className="overflow-x-auto">
          <table className="table table-zebra table-sm">
            ...
            <tbody>
              {filtered.map((d, i) => (
                <tr key={d.id}>
                  <td>{i + 1}</td><td className="font-medium">{d.name}</td>
                  <td className="ltr-num">{d.phone}</td>
                  <td className="text-error font-bold">{formatCurrency(d.balance)}</td>
                  <td><button className="btn btn-success btn-xs gap-1" onClick={() => { setShowPay(d); setPayAmount(d.balance); }}><DollarSign size={14} /> تسویه</button></td>
                </tr>
              ))}
              {filtered.length === 0 && <tr><td colSpan={5} className="text-center text-base-content/60 py-8">بدهکاری وجود ندارد 🎉</td></tr>}
            </tbody>
          </table>
        </div>
      </div></div>
      ...
      {showPay && (
        <div className="modal modal-open"><div className="modal-box">
          ...
          <p className="text-sm text-base-content/60 mb-3">بدهی: {formatCurrency(showPay.balance)}</p>
          <div className="space-y-3">
            ...
            <input className="input input-bordered w-full input-sm" placeholder="یادداشت" value={payNote} onChange={e => setPayNote(e.target.value)} />
          </div>
        ...
        </div><div className="modal-backdrop" onClick={() => setShowPay(null)} /></div>
      )}
      ...
      {editPay && (
        <div className="modal modal-open"><div className="modal-box">
          ...
          <div className="space-y-3">
            ...
            <div><label className="label label-text text-xs">روش</label>
        ...
        </div><div className="modal-backdrop" onClick={() => setEditPay(null)} /></div>
      )}
      ...
      {confirmDelete && (
        ...
        </div><div className="modal-backdrop" onClick={() => setConfirmDelete(null)} /></div>
      )}
    </div>
  );
}
