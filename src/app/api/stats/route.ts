...
export async function GET() {
  ...
  const salesToday = await db.execute({
    sql: "SELECT COALESCE(SUM(total - discount), 0) as val FROM invoices WHERE created_at LIKE ?",
    args: [today + '%'],
  });
  ...
  const catSales = await db.execute(`
    SELECT p.category, COALESCE(SUM(ii.total), 0) as total
  ...
  const topProds = await db.execute(`
    SELECT p.name, COALESCE(SUM(ii.quantity), 0) as qty
    FROM invoice_items ii JOIN products p ON p.id = ii.product_id
